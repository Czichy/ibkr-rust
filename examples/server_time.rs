...
fn main() {
    let client = Client::connect("10.15.10.25:1111", 100).expect("connection failed");
    ...
    let results = client.contract_details(&contract).unwrap();
...
//     let mut client = Builder::manual(1111, None)
//         .connect(8)
//         .await?
//         .disaggregated(ChannelWrapper { tx })
//         .await;

//     let spy = contract::new::<Stock>(&mut client,
// "BBG000BDTBL9".parse()?).await?;     let id = client
//         .req_historical_bar(
//             &spy,
